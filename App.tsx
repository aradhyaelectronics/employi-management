...
const App: React.FC = () => {
  ...
  const navItems = [
    { id: 'dashboard', label: 'Stats', icon: ICONS.Dashboard, roles: [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.EMPLOYEE] },
    { id: 'projects', label: 'Projects', icon: ICONS.Rocket, roles: [UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.EMPLOYEE] },
    { id: 'attendance', label: 'Punch', icon: ICONS.Time, roles: [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.EMPLOYEE] },
    { id: 'leaves', label: 'Leave', icon: ICONS.Shield, roles: [UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.EMPLOYEE] },
    { id: 'work', label: 'Logs', icon: ICONS.Work, roles: [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.EMPLOYEE] },
    { id: 'financials', label: 'Finance', icon: ICONS.Money, roles: [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.EMPLOYEE] },
    { id: 'users', label: 'Team', icon: ICONS.Users, roles: [UserRole.ADMIN, UserRole.SUPERVISOR] },
    { id: 'subscription', label: 'Billing', icon: ICONS.Rocket, roles: [UserRole.ADMIN] },
    { id: 'enterprise', label: 'Enterprises', icon: ICONS.Users, roles: [UserRole.SUPER_ADMIN] },
    { id: 'backend', label: 'Backend', icon: ICONS.Dashboard, roles: [UserRole.SUPER_ADMIN] },
  ];

  const visibleNavItems = navItems.filter(i => i.roles.includes(currentUser?.role || UserRole.EMPLOYEE));
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50">
      <aside className="hidden md:flex w-72 bg-white border-r flex-col p-8 sticky top-0 h-screen">
        ...
        <nav className="flex-1 mt-10 space-y-1.5">
          {visibleNavItems.map(i => (
            <button key={i.id} onClick={() => setActiveTab(i.id)} className={`w-full flex items-center space-x-4 px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-wider ${activeTab === i.id ? 'bg-[#0D47A1] text-white shadow-xl' : 'text-gray-400 hover:text-blue-900'}`}><i.icon className="w-4 h-4" /><span>{i.label}</span></button>
          ))}
        </nav>
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto mb-20 md:mb-0">
        <div className="max-w-7xl mx-auto">
          {activeTab === 'dashboard' && <Dashboard user={currentUser} state={state} />}
          ...
          {activeTab === 'users' && <UserManagement user={currentUser} state={state} addUser={addUser} />}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around items-center px-4 py-3 z-[100] shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        {visibleNavItems.slice(0, 5).map(i => (
          <button key={i.id} onClick={() => setActiveTab(i.id)} className={`flex flex-col items-center justify-center space-y-1 ${activeTab === i.id ? 'text-blue-700' : 'text-gray-300'}`}>
