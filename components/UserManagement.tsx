...
const UserManagement: React.FC<Props> = ({ user, state, addUser, updateUser, removeUser }) => {
  ...
  const handleCopyApk = () => {
    navigator.clipboard.writeText(state.apkUrl);
    setCopied(true);
  ...
  return (
    <div className="space-y-6">
       {/* APK Distribution Quick Link for Admin */}
       {(isAdmin || isSuper) && (
          <div className="bg-blue-600 p-6 rounded-[2rem] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-blue-900/10">
             <div className="flex items-center space-x-4">
                <div className="p-3 bg-white/10 rounded-2xl">
                   <ICONS.Android className="w-6 h-6 text-white" />
                </div>
                <div>
                   <h4 className="text-xs font-black uppercase tracking-widest">Employee App Setup</h4>
                   <p className="text-[10px] font-bold text-blue-100/70 uppercase mt-0.5">Share the Android APK with new field staff</p>
                </div>
             </div>
             <div className="flex gap-2 w-full md:w-auto">
                <button 
                  onClick={handleCopyApk}
                  className="flex-1 px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border border-white/10"
                >
                ...
                <a 
                  href={state.apkUrl} 
                  target="_blank" 
                  className="px-6 py-3 bg-white text-blue-600 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg"
                >
                  Download
                </a>
             </div>
